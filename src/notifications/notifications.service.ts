...
export class NotificationsService {
    ...
    async postNotification(
        ...
        try {
            const notification = await this.notificationRepository.findOneNotification(postId, reporterId);
            const reportedId = await this.notificationRepository.findReportedId(postId);
            await queryRunner.commitTransaction();

            if (!notification) {
                await this.notificationRepository.createNotification(postId, notificationContent, reporterId, reportedId);
    ...
    async cancelNotification(
        ...
        try {
            ...
            if (notification && notification.status === 'AWAIT') {
                await this.notificationRepository.cancelNotification(postId, reporterId);
    ...
    async getReporterNotification(
        ...
        try {
            return await this.notificationRepository.getReporterNotification(reporterId);
        } catch (error) {
    ...
    async getReportedNotification(
        ...
        try {
            return await this.notificationRepository.getReportedNotification(reportedId);
        } catch (error) {
    ...
    async acceptNotification(
        ...
        try {
            if (auth === 'admin' && status === 'ACCEPT') {
                return await this.notificationRepository.updateStatusNotification(notiId, status);
            } else {
